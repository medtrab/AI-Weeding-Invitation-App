...
interface AIResult { themeName: string; invitationText: string; musicSuggestion: string; decorativeStyle: string; colorPalette: Record<string,string>; }
  ...
  return (
    <section id="ai-gen" className="py-28 bg-gradient-to-b from-deep to-[#100E0A]">
      <div className="max-w-3xl mx-auto px-6">
        ...
        <motion.div className="bg-gold/[0.04] border border-gold/25 p-6 mb-4" initial={{ opacity:0, y:24 }} whileInView={{ opacity:1, y:0 }} viewport={{ once:true }}>
          ...
          <div className="flex flex-wrap gap-2 mb-5">
            {PROMPTS.map((p, i) => (
              <button key={p.label} onClick={() => { setActive(i); setPrompt(p.text); }}
                className={`px-4 py-2 text-xs tracking-[0.1em] border transition-all ${i === active ? "border-gold text-gold bg-gold/10" : "border-gold/20 text-cream/50 hover:border-gold/50 hover:text-cream/80"}`}>
                {p.label}
              </button>
            ))}
          </div>
        ...
        <button onClick={generate} disabled={loading}
          className="w-full py-4 bg-gradient-to-r from-gold to-[#A8893A] text-deep text-xs uppercase tracking-[0.25em] font-medium hover:brightness-110 transition-all disabled:opacity-60 relative overflow-hidden">
          ...
          {result && (
            <motion.div className="mt-6 grid grid-cols-2 gap-3" initial={{ opacity:0, y:16 }} animate={{ opacity:1, y:0 }} exit={{ opacity:0 }}>
              ...
              <ResultCell label="Color Palette">
                <div className="flex gap-2 mt-1">{Object.values(result.colorPalette).slice(0,5).map((c,i) => <div key={i} className="w-7 h-7 rounded-full" style={{ background: c }} />)}</div>
              </ResultCell>
    ...
    <div className="bg-deep text-cream min-h-screen">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-8 py-4 backdrop-blur-xl bg-deep/70 border-b border-gold/10">
        ...
        <div className="hidden md:flex gap-6 text-[11px] uppercase tracking-[0.15em]">
          {[["#ai-gen","Generator"],["#themes","Themes"],["#features","Features"],["#pricing","Pricing"]].map(([href,label]) => (
      ...
      <section className="relative min-h-screen flex items-center justify-center text-center px-6 pt-24 pb-16 overflow-hidden">
        ...
        <div className="relative z-10 max-w-4xl">
          {[
            <motion.p key="ey" className="text-[11px] uppercase tracking-[0.35em] text-gold mb-6" initial={{ opacity:0, y:24 }} animate={{ opacity:1, y:0 }} transition={{ duration:0.9 }}>
              AI-Powered Event Invitations
            </motion.p>,
            <motion.h1 key="h1" className="font-cormorant text-[clamp(3rem,7vw,6.5rem)] font-light leading-[1.05] mb-6" initial={{ opacity:0, y:24 }} animate={{ opacity:1, y:0 }} transition={{ duration:0.9, delay:0.15 }}>
              Invitations that<br /><em className="italic text-gold">move hearts</em>
            </motion.h1>,
            <motion.p key="sub" className="text-sm text-cream/50 max-w-md mx-auto mb-10 leading-relaxed tracking-wide" initial={{ opacity:0, y:24 }} animate={{ opacity:1, y:0 }} transition={{ duration:0.9, delay:0.3 }}>
              Generate breathtaking, personalized event invitations with AI. Every detail crafted for your most memorable moments.
            </motion.p>,
            <motion.div key="cta" className="flex gap-4 justify-center flex-wrap" initial={{ opacity:0, y:24 }} animate={{ opacity:1, y:0 }} transition={{ duration:0.9, delay:0.45 }}>
              ...
              <a href="#themes" className="px-10 py-4 border border-cream/20 text-cream text-xs uppercase tracking-[0.25em] hover:border-gold hover:text-gold transition-all">
                Explore Themes
              </a>
            </motion.div>,
          ]}
          ...
          <div className="grid md:grid-cols-3 gap-px bg-gold/15">
            {PLANS.map((plan, i) => (
              <motion.div key={plan.name} className={`p-8 ${plan.featured ? "bg-gradient-to-b from-[#1A1608] to-[#141009]" : "bg-deep"}`}
                ...
                <Link href="/register"
                  className={`block w-full text-center py-3 text-[11px] uppercase tracking-[0.2em] transition-all ${plan.featured ? "bg-gold text-deep hover:bg-gold-light" : "border border-gold/30 text-gold hover:bg-gold hover:text-deep"}`}>
                  {plan.cta}
                </Link>
              </motion.div>
            ))}
      ...
      <footer className="bg-[#050402] border-t border-gold/10 py-12 px-6">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 mb-8">
            {[
              { title:"Product",  links:[["#ai-gen","AI Generator"],["#themes","Themes"],["#features","Builder"],["#pricing","Pricing"]] },
